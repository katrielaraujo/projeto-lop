...
function preload() {
  song = loadSound('axel-f-official-video.mp3	')
...
function draw() {
  ...
  function mons(h) {
    if (dificuldadeF === false) {
      correcao[h] = correcao[h] + 0.01;
      xMonstro[h] = height * noise(correcao[h])
      yMonstro[h] = yMonstro[h] + 1
      fill(255, 20, 20);
      monstro[h] = ellipse(xMonstro[h], yMonstro[h], 20, 20);
      if (Math.pow(((personagemX + 10) - xMonstro[h]), 2) + Math.pow(((-personagemY - 10) + yMonstro[h]), 2) <= 700) {
        morte = true;
        fase = -1;
      }
      if (yMonstro[h] > 600) {
        yMonstro[h] = -150;
        xMonstro[h] = random(20, 280);
        correcao[h] = 150;
      }
    }
  }
  ...
  if (fase == 0) {
    ...
    if (pontos < 50) {
      ...
      if (pontos < 80) {
        textSize(20);
        text('Espere até os pontos\npararem de crescer', 70, 150);
        gravity = 0;
        personagemX = 140;
        pontos += 1 / 15;
        if (pontos == 80) {
      ...
      } else if (pontos > 120 && pontos < 250 || dificuldadeF === false) {
        mons(0);
        mons(1);
      } else if (pontos > 250 && pontos < 500 || dificuldadeN === true) {
        mons(0);
        mons(1);
        mons(2);
      } else if (dificuldadeD === true) {
        mons(0);
        mons(1);
        mons(2);
        mons(3);
      ...
      textSize(20);
      text('voce ganhou\n estou sem tempo\npara emplementar mais coisas T.T', 10, 50);
